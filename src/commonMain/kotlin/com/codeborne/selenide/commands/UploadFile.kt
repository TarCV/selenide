...
class UploadFile : Command<Path> {
    ...
    @kotlin.time.ExperimentalTime
    protected suspend fun uploadFiles(config: Config, inputField: org.openqa.selenium.WebElement, fileNames: String) {
        val stopwatch = Stopwatch(config.timeout())
        do {
            try {
                inputField.sendKeys(fileNames)
                break
            } catch (notInteractable: org.openqa.selenium.ElementNotInteractableException) {
