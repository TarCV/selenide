...
open class WebPageSourceExtractor : PageSourceExtractor {
    ...
    override suspend fun extract(config: Config, driver: org.openqa.selenium.WebDriver, fileName: String): Path {
        return extract(config, driver, fileName, true)
    }

    @ExperimentalFileSystem
    private suspend fun extract(config: Config, driver: org.openqa.selenium.WebDriver, fileName: String, retryIfAlert: Boolean): Path {
    ...
    @ExperimentalFileSystem
    private suspend fun retryingExtractionOnAlert(config: Config, driver: org.openqa.selenium.WebDriver, fileName: String, e: Exception) {
        try {
