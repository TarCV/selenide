...
open class WindowsCloser {
    open suspend fun <T: Any> runAndCloseArisedWindows(webDriver: org.openqa.selenium.WebDriver, lambda: suspend () -> T): T {
    ...
    private suspend fun closeArisedWindows(webDriver: org.openqa.selenium.WebDriver, originalWindowHandle: String, windowsBefore: Set<String>) {
    ...
    private fun newWindows(webDriver: org.openqa.selenium.WebDriver, windowsBefore: Set<String>): Set<String> {
    ...
    private suspend fun closeWindows(webDriver: org.openqa.selenium.WebDriver, windows: Set<String>) {
    ...
    private suspend fun closeWindow(webDriver: org.openqa.selenium.WebDriver, window: String) {
        log.info { "  Let's close $window" }
        try {
            webDriver.switchTo().window(window)
            webDriver.close()
        } catch (windowHasBeenClosedMeanwhile: org.openqa.selenium.NoSuchWindowException) {
