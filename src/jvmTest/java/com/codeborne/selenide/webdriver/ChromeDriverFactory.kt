...
open class ChromeDriverFactory : AbstractDriverFactory() {
    ...
    protected fun <DS : DriverService?, B : DriverService.Builder<DS, *>> withLog(config: Config, dsBuilder: B): DS {
        if (config.webdriverLogsEnabled()) {
            dsBuilder.withLogFile(webdriverLog(config).toFile())
        }
    ...
    @ExperimentalFileSystem
    @CheckReturnValue
    protected fun prefs(browserDownloadsFolder: Path?): Map<String, Any> {
        ...
        if (browserDownloadsFolder != null) {
            chromePreferences["download.default_directory"] = browserDownloadsFolder.toFile().absolutePath
        }
